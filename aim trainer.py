...
class Asteroid:
    def __init__(self, canvas):
        self.canvas = canvas
        self.id = canvas.create_oval(0, 0, ASTEROID_SIZE, ASTEROID_SIZE, fill='gray')
...
class AsteroidDestroyerGame:
    ...
    def on_click(self, event):
        ...
        for asteroid in self.asteroids:
            if asteroid.id == asteroid_id:
                ...
                if self.is_collision(event.x, event.y, asteroid.x, asteroid.y):
    ...
    def is_collision(self, x, y, ax, ay):
        x_distance = abs((ax + ASTEROID_SIZE / 2) - x)
        y_distance = abs((ay + ASTEROID_SIZE / 2) - y)
        return x_distance < ASTEROID_SIZE / 2 and y_distance < ASTEROID_SIZE / 2
